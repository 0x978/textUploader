...
const EditGroupSelect: FC<EditGroupSelectProps> = ({groups,fetchedPaste,handleNewGroup,handleGroupChange}) => {
    return(
        <div className="space-y-2">
            ...
            <button className="bg-puddlePurple w-44 h-10 hover:text-green-400 hover:scale-110 transition duration-300 active:translate-y-1"
                    onClick={() => void handleNewGroup()}>Create new Group</button>

            {groups.map((group,i) => { // map through each group, passed as props.
                if (group !== fetchedPaste.group) { // we don't want the user to be able to select the group that the paste already belongs to.
                    return (
