    ...
    <>
      <main className="flex h-screen text-center bg-deepPurple text-superCoolEdgyPurple">
        <div className="m-auto">
          ...
          {isLoading ? // if fetching categories, display as such.
            ...
            <div>
              {paginatedGroups.length === 0 ? // if user has no pastes, display as such
                ...
                <GroupDisplay paginatedGroups={paginatedGroups} groupMap={groupMap} redirect={redirect} />
              }
            </div>
          }


          <div className="my-5 space-x-10 "> {/* Div responsible for increment, decrement buttons for pagination */}
            <button className="w-40 bg-puddlePurple p-2 hover:-translate-y-1 transition duration-300" onClick={() => {
          ...
          <div>
            <button className={"bg-puddlePurple p-2 active:translate-y-1.5 hover:text-green-300"} onClick={() => void router.push("/settings")} >User Settings</button>
          </div>
