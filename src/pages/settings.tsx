      ...
      <div className="m-auto space-y-5">
        ...
        <div className="space-y-2">
          ...
          <div className={"my-2"}>
            {toggleKey ?
              <h1>{user.id}</h1> :
              <button onClick={() => setToggleKey(true)} className={"bg-puddlePurple p-2 "}>Show Key </button>}
          </div>
        ...
        <div className={"flex flex-col items-center space-y-3"}>
          <button className={"bg-puddlePurple w-40 p-2 hover:text-red-300 active:translate-y-1.5"} onClick={() => void logout()}>Logout</button>
          <button className={"bg-puddlePurple w-40 p-2 hover:text-green-300 active:translate-y-1.5"} onClick={() => void router.push("/groupSelect")}>Return</button>
        </div>
